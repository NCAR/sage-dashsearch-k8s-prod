#!/bin/bash
  ...
  # Run 'git fetch' on CKAN source; needed by repo_info plugin
  cd ${SRC_DIR}/ckan
  git fetch
  cd
  ...
  if [ ! -d "${SRC_DIR}/NCAR" ]; then
     ...
     mkdir NCAR
     cd NCAR
     ln -s ${SRC_EXTENSIONS_DIR}/ckanext-dsetsearch .
     cd
  fi
  ...
  ln -sf ${SRC_EXTENSIONS_DIR}/ckanext-dsetsearch/solr/schema.solr-spatial-2.0.xml schema.xml
